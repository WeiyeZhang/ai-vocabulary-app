  ...
  if (items.length === 0) {
    return (
      <div className="text-center text-gray-400 p-8 bg-gray-800/50 rounded-2xl">
        <h2 className="text-2xl font-bold mb-2">Your deck is empty.</h2>
  ...
  return (
    <div className="w-full max-w-xl mx-auto flex flex-col items-center space-y-6">
      <div className="w-full h-64 sm:h-80 md:h-96" onClick={!isFlipped ? handleFlip : undefined}>
        {currentItem ? <Flashcard item={currentItem} isFlipped={isFlipped} /> : <div className="w-full h-full bg-gray-700 rounded-2xl"></div>}
      </div>
        ...
        {isFlipped && (
          <div className="w-full max-w-lg space-y-6 animate-fade-in">
            ...
            <div className="bg-gray-800/50 p-4 rounded-xl border border-gray-700">
              <h4 className="font-bold text-gray-200 mb-2">Feynman Technique: Explain it simply</h4>
              <textarea
                value={explanation}
                onChange={(e) => setExplanation(e.target.value)}
                placeholder="Explain this concept in your own words..."
                className="w-full bg-gray-700 border border-gray-600 rounded-md py-2 px-3 text-white placeholder-gray-400 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition h-24"
              />
